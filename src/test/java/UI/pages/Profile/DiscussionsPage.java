...
public class DiscussionsPage extends MainPage {
    ...
    private final static By MORE_ACTIONS_BUTTON = By.xpath(".//div[@class='ui_actions_menu_icons'][./div[@class='groups_actions_icons']]");
    ...
    public void createTopic(String title, String text) {
        findElement(TITLE_FIELD).sendKeys(title);
        findElement(TEXT_FIELD).sendKeys(text);
        findElement(CREATE_TOPIC_BUTTON).click();
    }
    ...
    public void verifyTopicWasDeleted() {
        Assert.assertTrue(findElement(MESSAGE).getText().contains("Тема удалена.\n" +
    ...
    public void deleteGroup() throws InterruptedException {
        WebElement group = findElement(By.xpath("//div[@id='groups_list_groups']//div[contains(@class,'group_list_row')]"));
        clickWithJavascript(findElement(MORE_ACTIONS_BUTTON));
        findElement(By.xpath("//a[text()='Отписаться']")).click();
        Thread.sleep(1000);
        Assert.assertTrue(group.getAttribute("class").contains("deleted"), "пользователь не отписался от группы");
    }
}
